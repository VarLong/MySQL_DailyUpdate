...
select concat(first_name, ' ', last_name) as e_name, max(salary) as max_salary from employee;

select max(salary) as max_salary, job_id from employee group by job_id having max_salary>30000 order by job_id desc;
use company;

select * from department;
select * from employee;

select concat(first_name, " ", last_name) as full_nema, department_name from employee, department where employee.department_id=department.department_id and department.department_id is not null;
